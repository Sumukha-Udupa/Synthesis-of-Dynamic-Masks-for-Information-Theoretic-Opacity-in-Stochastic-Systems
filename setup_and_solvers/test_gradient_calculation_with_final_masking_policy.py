...
class PrimalDualPolicyGradientTest:
    def __init__(self, hmm, iter_num=1000, batch_size=1, V=100, T=10, eta=1, kappa=0.1, epsilon=0,
                 sensor_cost_normalization=1, exp_number=0):
        ...
        self.sensor_cost_normalization = sensor_cost_normalization
        ...
        self.theta_torch = torch.nn.Parameter(
            torch.empty(self.num_of_aug_states, self.num_of_maskin_actions, dtype=torch.float32, device=device,
                        requires_grad=True))

        # Initialize the theta_torch with the final state masking policy.
        pre_for_secret_states_under_A = {3, 10, 9, 8}
        pre_for_secret_states_under_C = {22, 29, 23}
        specific_values = list([])
        for i in range(self.num_of_aug_states):
            if self.hmm.augmented_states[i][0] in pre_for_secret_states_under_A:
                specific_values.append([1.0, 0, 0])
            elif self.hmm.augmented_states[i][0] in pre_for_secret_states_under_C:
                specific_values.append([0, 1.0, 0])
            else:
                specific_values.append([0, 0, 1.0])

        # # The below is for the policy that masks any sensor when the next state is within that sensor.
        # pre_for_secret_states_under_A = {3, 10, 9, 8, 4, 2}  # I am temporarily writing these sets manually for now.
        # pre_for_secret_states_under_C = {22, 29, 23}
        # pre_for_secret_states_under_B = {27, 21, 28, 34, 16}
        # pre_for_secret_states_under_D = {0, 6, 7, 12, 13, 14, 18, 20}
        # specific_values = list([])
        # for i in range(self.num_of_aug_states):
        #     if self.hmm.augmented_states[i][0] in pre_for_secret_states_under_A:
        #         specific_values.append([1.0, 0, 0, 0, 0])
        #     elif self.hmm.augmented_states[i][0] in pre_for_secret_states_under_C:
        #         specific_values.append([0, 0, 1.0, 0, 0])
        #     elif self.hmm.augmented_states[i][0] in pre_for_secret_states_under_B:
        #         specific_values.append([0, 1.0, 0, 0, 0])
        #     elif self.hmm.augmented_states[i][0] in pre_for_secret_states_under_D:
        #         specific_values.append([0, 0, 0, 1.0, 0])
        #     else:
        #         specific_values.append([0, 0, 0, 0, 1.0])

        initial_values = torch.tensor(specific_values, dtype=torch.float32, device=device)
        self.theta_torch.data = initial_values
    ...
    def sample_action_torch(self, state):
        ...
        action_probs = self.theta_torch[state_indx]
    ...
    def solver(self):
        ...
        for i in range(self.iter_num):
            ...
            with torch.no_grad():
                ...
                self.theta_torch = self.theta_torch
        ...
        with open('../entropy_values_final_state_masking_policy2.pkl', 'wb') as file:
            pickle.dump(self.entropy_list, file)

        with open('../value_function_list_final_state_masking_policy2.pkl', 'wb') as file:
        ...
        # with open('../final_masking_policy_for_fina_state_masking_policy_all_sensors.pkl', 'wb') as file:
        ...
        mean_approximate_cost = sum(self.threshold_list)/len(self.threshold_list)
        print("The mean approximate cost = ", mean_approximate_cost)
