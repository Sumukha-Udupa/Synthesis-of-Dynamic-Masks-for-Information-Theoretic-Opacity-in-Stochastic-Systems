...
class PrimalDualPolicyGradientTest:
    def __init__(self, hmm, iter_num=1000, batch_size=1, V=100, T=10, eta=1, kappa=0.1, epsilon=0):
        ...
        # Adam parameters for adaptive learning rate.
        self.beta1 = 0.9
        self.beta2 = 0.999
        self.epsilon = 1e-8
        self.m = torch.zeros_like(self.theta_torch, dtype=torch.float32, device=device)
        self.v = torch.zeros_like(self.theta_torch, dtype=torch.float32, device=device)
    ...
    def solver(self):
        ...
        for i in range(self.iter_num):
            ...
            # Adam updates.
            with torch.no_grad():
                self.m = self.beta1 * self.m + (1 - self.beta1) * grad_L
                self.v = self.beta2 * self.v + (1 - self.beta2) * (grad_L ** 2)
            ...
            with torch.no_grad():
                # decay_factor = 0.1
                # decay_steps = 200
                # self.eta = self.eta * (decay_factor ** (i // decay_steps))
                self.theta_torch = self.theta_torch + self.eta * m_hat / (torch.sqrt(v_hat) + self.epsilon)
